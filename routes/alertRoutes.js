...
//    // Send email for critical alerts with PDF to all family users
...
//         const todayVitals = allData.filter(e => e.type === "vitals" && new Date(e.date).toDateString() === today);
//         const todayMeds   = allData.filter(e => e.type === "medication");

//         const patientName = todayVitals[0]?.patientName || "Patient";
// console.log("🔍 Patient name from vitals:", patientName);

// const allFamilyUsers = await User.find({ role: "family" });
// console.log("👨‍👩‍👧 All family users:", allFamilyUsers.map(u => ({ 
//   username: u.username, 
//   patients: u.patients, 
...
//   patients: { $in: [new RegExp(`^${patientName.trim()}$`, 'i')] }
// });
// console.log("✅ Matched family users:", familyUsers.map(u => u.username));
// const recipients = familyUsers.map(u => u.notifyEmail);
...
//           status: type === "emergency" ? "Critical" : "Warning"
//         });
...
//         // Send email for missed medication to correct family
//         const User = require("../models/User");
//         const missedFamilyUsers = await User.find({
//           role: "family",
//           notifyEmail: { $exists: true, $ne: null },
//           patients: { $in: [new RegExp(`^${p.patientName.trim()}$`, 'i')] }
//         });
//         const missedRecipients = missedFamilyUsers.map(u => u.notifyEmail);
